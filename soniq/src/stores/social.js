...
export const useSocialStore = defineStore('social', () => {
  ...
  async function loadFriends() {
    try {
      const res = await fetch(`${BASE_URL}/api/friends`, CREDS)
  ...
  async function sendFriendRequest(username) {
    const res = await fetch(`${BASE_URL}/api/friends/request`, {
      method: 'POST', ...CREDS, headers: JSON_HEADERS,
  ...
  async function removeFriend(userId) {
    await fetch(`${BASE_URL}/api/friends/${userId}`, { method: 'DELETE', ...CREDS })
  ...
  async function loadGroups() {
    try {
      const res = await fetch(`${BASE_URL}/api/groups`, CREDS)
  ...
  async function leaveGroup(id) {
    await fetch(`${BASE_URL}/api/groups/${id}/leave`, { method: 'DELETE', ...CREDS })
  ...
  async function loadConversations() {
    try {
      const res = await fetch(`${BASE_URL}/api/conversations`, CREDS)
  ...
  async function loadMessages(targetId) {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/messages/${targetId}`, CREDS)
